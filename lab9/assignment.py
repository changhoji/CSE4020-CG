...
angle = [glm.degrees(0), glm.degrees(0), glm.degrees(0)]
...
def key_callback(window, key, scancode, action, mods):
    global g_cam_ang, g_cam_height, angle
    if key==GLFW_KEY_ESCAPE and action==GLFW_PRESS:
        ...
        if action==GLFW_PRESS or action==GLFW_REPEAT:
            if key==GLFW_KEY_1:
                ...
                angle[0] += glm.radians(10)
            elif key==GLFW_KEY_Z:
                angle[0] -= glm.radians(10)
            elif key==GLFW_KEY_S:
                angle[1] += glm.radians(10)
            elif key==GLFW_KEY_X:
                angle[1] -= glm.radians(10)
            elif key==GLFW_KEY_D:
                angle[2] += glm.radians(10)
            elif key==GLFW_KEY_C:
                angle[2] -= glm.radians(10)
            elif key==GLFW_KEY_V:
                angle = [0, 0, 0]
...
def main():
    ...
    while not glfwWindowShouldClose(window):
        ...
        Rx = glm.rotate(a, (0,0,1))
        Ry = glm.rotate(b, (1,0,0))
        Rz = glm.rotate(r, (0,0,1))
        M = glm.mat4(Rx * Ry * Rz)
